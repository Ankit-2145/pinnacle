"use client";

import Silk from "@/components/silk";
import { ChevronRight } from "lucide-react";
import { AppPrimaryButton } from "../app-button";

export const CTABanner = () => {
  return (
    <div className="max-w-6xl mx-auto overflow-hidden rounded-lg relative border font-workSans">
      {/* Text Content */}
      <div className="relative h-full flex flex-col items-start justify-start px-4 py-8">
        <div className="absolute inset-0 z-0">
          <Silk
            speed={5}
            scale={1}
            color="#228ae7"
            noiseIntensity={0}
            rotation={0}
          />
        </div>
        <div className="z-10">
          <h3 className="text-2xl font-medium text-background">
            Still figuring out how to build your website?
          </h3>
          <p className="text-base max-w-xl mt-2 text-background">
            You could spend another weekend Googling tutorials — or partner with
            a team that gets it right the first time. We&apos;re here to make it
            easier, faster, and better.
          </p>
          <div className="mt-6 flex flex-col sm:flex-row gap-4 items-center">
            <AppPrimaryButton
              label="Ready to get yours?"
              href="/"
              icon={ChevronRight}
            />
          </div>
        </div>
      </div>
    </div>
  );
};
